#!/usr/bin/env python3
"""
Test script to verify 0.4.0 structure without requiring dependencies.
"""
import os
import sys
...
def test_file_structure():
    ...
    print("Testing 0.4.0 file structure...")
...
def test_version_numbers():
    ...
    if Path(pyproject_file).exists():
        ...
        if 'version = "0.4.0"' in content:
            print("✅ pyproject.toml version updated to 0.4.0")
            results["pass"] += 1
        else:
            print("❌ pyproject.toml version not updated")
            results["fail"] += 1
            results["errors"].append("pyproject.toml version not 0.4.0")
    else:
    ...
    if Path(init_file).exists():
        ...
        if '__version__ = "0.4.0"' in content:
            print(f"✅ __init__.py version updated to 0.4.0")
            results["pass"] += 1
        else:
            print(f"❌ __init__.py version not updated")
            results["fail"] += 1
            results["errors"].append("__init__.py version not 0.4.0")
...
def main():
    ...
    print("=" * 60)
    print("reportHanter 0.4.0 Structure Verification")
    print("=" * 60)
    ...
    if total_results["fail"] == 0:
        print("\n🎉 All tests passed! reportHanter 0.4.0 structure is correct.")
        return True
    else:
