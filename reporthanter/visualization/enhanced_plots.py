...
class EnhancedKrakenPlotGenerator(BasePlotGenerator):
    ...
    def _create_enhanced_bar_chart(
        self, data: pd.DataFrame, title: str, unclassified_pct: float
    ) -> alt.Chart:
        ...
        bars = (
            ...
            .encode(
                ...
                y=alt.Y("name:N", sort="-x", axis=alt.Axis(title=None, labelLimit=200)),
                color=alt.Color(
                    "domain:N",
                    scale=domain_colors,
        ...
        # Add hover highlighting. Altair 5: selection_single -> selection_point
        # with empty=False, nearest=True; add_selection -> add_params.
        hover = alt.selection_point(on="mouseover", empty=False, nearest=True)
        ...
        text = (
            alt.Chart(data)
            .mark_text(align="left", baseline="middle", dx=5, fontSize=10, fontWeight="normal")
            .encode(
                x=alt.X("percent:Q"),
    ...
    def _create_donut_chart(self, data: pd.DataFrame, title: str) -> alt.Chart:
        ...
        donut = (
            ...
            .mark_arc(innerRadius=60, outerRadius=120, stroke="white", strokeWidth=2)
            .encode(
        ...
        center_text = (
            alt.Chart(pd.DataFrame([{"text": f"Total\n{data['count_clades'].sum():.0f} reads"}]))
            .mark_text(fontSize=14, fontWeight="bold", align="center")
            .encode(text="text:N")
        )
    ...
    def _create_treemap_chart(self, data: pd.DataFrame, title: str) -> alt.Chart:
        ...
        circles = (
            alt.Chart(plot_data, title=title)
            .mark_circle(stroke="white", strokeWidth=2, opacity=0.8)
            .encode(
                x=alt.X("x:O", axis=None, scale=alt.Scale(padding=0.1)),
                y=alt.Y("y:O", axis=None, scale=alt.Scale(padding=0.1)),
                size=alt.Size("percent:Q", scale=alt.Scale(range=[200, 2000]), legend=None),
                color=alt.Color(
                    "domain:N",
                    scale=alt.Scale(
                        domain=["Viruses", "Bacteria", "Archaea", "Eukaryota"],
    ...
    def _create_dashboard_view(
        ...
        stats_data = pd.DataFrame(
            [
                ...
                {
                    "metric": "Top Species %",
                    "value": data["percent"].iloc[0] if len(data) > 0 else 0,
                },
        ...
        stats_chart = (
            alt.Chart(stats_data, title="Summary Statistics")
            .mark_bar(cornerRadius=2, color="#1f77b4", opacity=0.7)
            .encode(
...
class EnhancedQualityPlotGenerator(BasePlotGenerator):
    ...
    def _create_gauge_chart(self, data: pd.DataFrame, title: str) -> alt.Chart:
        ...
        background = (
            ...
            .encode(x=alt.X("metric:N", axis=None), theta=alt.value(3.14))
        )

        # Value arcs
        value_arcs = (
            alt.Chart(data)
            .mark_arc(innerRadius=50, outerRadius=80, theta2=0)
            .encode(
                x=alt.X("metric:N", axis=None),
                theta=alt.Theta("value:Q", scale=alt.Scale(range=[0, 3.14])),
...
class ResponsiveDashboard:
    ...
    def create_classification_dashboard(
        ...
        dashboard = alt.vconcat(
            alt.Chart().mark_text(fontSize=18, fontWeight="bold").encode(text=alt.value(title)),
            top_row,
            comparison_plot,
    ...
    def _create_comparison_data(
        kraken_data: pd.DataFrame, kaiju_data: pd.DataFrame
    ) -> pd.DataFrame:
