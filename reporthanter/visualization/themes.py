    ...
    def get_taxonomy_theme() -> dict[str, Any]:
        ...
        return {
            "config": {
                ...
                "legend": {"labelFontSize": 11, "titleFontSize": 13, "symbolSize": 100},
                "title": {"fontSize": 16, "fontWeight": "normal", "color": "#333333"},
            }
        }
    ...
    def get_quality_theme() -> dict[str, Any]:
        ...
        return {
            "config": {
                ...
                "title": {"fontSize": 15, "anchor": "start", "offset": 10},
            }
        }
...
class InteractiveFeatures:
    ...
    @staticmethod
    def create_hover_selection(name: str = "hover") -> alt.Parameter:
        """Create hover selection for highlighting.

        Altair 5 deprecated ``selection_single``; ``selection_point`` with
        ``empty=False`` and ``nearest=True`` reproduces the single-hover
        behaviour we want here.
        """
        return alt.selection_point(name=name, on="mouseover", empty=False, nearest=True)

    @staticmethod
    def create_click_selection(name: str = "click") -> alt.Parameter:
        """Create click selection for filtering.

        Altair 5 deprecated ``selection_multi``; ``selection_point`` with
        ``fields`` and ``toggle=True`` (the default) is the replacement.
        """
        return alt.selection_point(name=name, fields=["category"])
...
class ChartEnhancements:
    ...
    def create_enhanced_bar_chart(
        ...
        bars = chart.mark_bar(cornerRadius=2, opacity=0.8, stroke="white", strokeWidth=1).encode(
        ...
        # Add hover effects. Altair 5 renamed ``add_selection`` to
        # ``add_params`` since selections are now a flavour of parameter.
        hover = InteractiveFeatures.create_hover_selection()
        bars = bars.add_params(hover).encode(
    ...
    def create_donut_chart(
        data, theta_field: str, color_field: str, title: str = "Distribution"
    ) -> alt.Chart:
        """Create a donut chart for proportional data."""
        return (
            alt.Chart(data, title=title)
            .mark_arc(innerRadius=50, outerRadius=100, stroke="white", strokeWidth=2)
            .encode(
    ...
    def create_stacked_area_chart(
        data, x_field: str, y_field: str, color_field: str, title: str = ""
    ) -> alt.Chart:
    ...
    @staticmethod
    def create_heatmap(
        data, x_field: str, y_field: str, color_field: str, title: str = "Correlation Matrix"
    ) -> alt.Chart:
...
class StatisticalOverlays:
    ...
    def add_mean_line(chart: alt.Chart, data_field: str) -> alt.LayerChart:
        ...
        mean_line = (
            ...
            .transform_aggregate(mean_value=f"mean({data_field})")
            .encode(
    ...
    def add_confidence_interval(
        chart: alt.Chart, data_field: str, confidence: float = 0.95
    ) -> alt.LayerChart:
        ...
        ci_band = (
            ...
            .transform_aggregate(
                lower=f"ci0({data_field}, {confidence})", upper=f"ci1({data_field}, {confidence})"
            )
            .encode(y=alt.Y("lower:Q"), y2=alt.Y("upper:Q"))
        )
    ...
    def add_threshold_lines(
        chart: alt.Chart, thresholds: dict[str, float], colors: dict[str, str] | None = None
    ) -> alt.LayerChart:
        ...
        for name, value in thresholds.items():
            line = (
                alt.Chart()
                .mark_rule(color=colors.get(name, "gray"), strokeDash=[3, 3], size=1)
                .encode(y=alt.datum(value), tooltip=alt.value(f"{name.title()}: {value}"))
            )
            layers.append(line)
